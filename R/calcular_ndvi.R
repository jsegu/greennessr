...
#'
#' @description  Extrae el valor del índice de vegetación normalizado (NDVI) para cada sección censal
#' ponderado por la ubiacación de las parcelas catastrales y el número de viviendas.
#' Para ello utiliza los datos de una imagen del satelite Landsat, a la que se le realiza el cálculo del
#' NDVI, una capa de polígonos correspondiente a las secciones censales y el mapa catastras.
#' Por defecto el método de extracción es la media de píxeles de cada polígono,
#' este método se puede modificar con el parámetro
#' \code{calculo}.
#'
#' @param dir_img Carácter: Directorio donde se encuentra la imagen Landsat, tiene que estar descomprimida.
#' @param sc Objeto espacial \code{(SpatialPolygonsDataFrame / sf):} con las geometrías poligonales de
...
#'
#' @export
calcular_ndvi_cat <- function(dir_img, sc, catastro, calculo = mean, dev_raster = FALSE, landsat8 = FALSE){
